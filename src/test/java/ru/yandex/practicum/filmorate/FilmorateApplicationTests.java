...
class FilmorateApplicationTests {
    ...
    public void createUserTest() throws ValidationException {
        User user1 = User.builder().email("@Email").login("login").name("Name")
                .birthday(LocalDate.of(2010, 10, 10)).build();
    ...
    void createAndFindFriendshipByUserIdsTest() {
        Friends friendship = Friends.builder().userId1(1).userId2(2).statusId(1).build();
        friendDbStorage.create(friendship);

        Assertions.assertThat(friendDbStorage.findFriendsByUserIds(1, 2)).isPresent();
        Assertions.assertThat(friendDbStorage.findFriendsByUserIds(2, 1)).isEmpty();
    }

    @Order(220)
    @Test
    void getFriendIdsByUserIdTest() {
        Assertions.assertThat(friendDbStorage.getFriendIdsByUserId(1)).hasSize(1);
        Assertions.assertThat(friendDbStorage.getFriendIdsByUserId(2)).hasSize(0);
    }

    @Order(230)
    @Test
    void updateFriendshipTest() {
        Friends friendship = Friends.builder().userId1(3).userId2(2).statusId(1).build();
        friendDbStorage.create(friendship);
        friendship = Friends.builder().id(2).userId1(2).userId2(3).statusId(1).build();
        friendDbStorage.update(friendship);
        Assertions.assertThat(friendDbStorage.findFriendsByUserIds(2, 3)).isPresent();
    }

    @Order(240)
    @Test
    void getCommonFriendIdsByUserIdsTest() {
        Friends friendship = Friends.builder().id(1).userId1(1).userId2(3).statusId(1).build();
        friendDbStorage.update(friendship);
        Assertions.assertThat(friendDbStorage.getCommonFriendIdsByUserIds(1, 2)).hasSize(1);
        Assertions.assertThat(friendDbStorage.getCommonFriendIdsByUserIds(2, 1)).hasSize(1);
    }

    @Order(250)
    @Test
    void deleteTest() {
        friendDbStorage.delete(1, 2);
        Assertions.assertThat(friendDbStorage.findFriendsByUserIds(1, 2)).isEmpty();
    }
    ...
    void createAndFindAllFilmTest() {
        ...
        Film film1 = Film.builder().name("Film").description("New film decription").duration(190)
                ...
                .mpa(mpa1)
                .build();
        Film createdFilm1 = filmDbStorage.create(film1).orElseThrow(() -> new NotFoundException("Film not created"));
        ...
        Film film2 = Film.builder().name("New film").description("New film about friends").duration(190)
                ...
                .genres(genres2)
                .build();
        Film createdFilm2 = filmDbStorage.create(film2).orElseThrow(() -> new NotFoundException("Film not created"));
    ...
    void createFilmLikeTest() {
        Assertions.assertThat(filmLikeDbStorage.createFilmLike(1, 2)).isPresent()
                .hasValueSatisfying(filmLike ->
                        Assertions.assertThat(filmLike).hasFieldOrPropertyWithValue("userId", 1L)
                );
        Assertions.assertThat(filmLikeDbStorage.createFilmLike(2, 2)).isPresent()
                .hasValueSatisfying(filmLike ->
                        Assertions.assertThat(filmLike).hasFieldOrPropertyWithValue("userId", 2L)
                );
    }
